...
FindNewWindow()
{
  ...
  do
	...
	if (!data_loaded[i])
      ...
      if ((!query_use_win(mini)) || (!unload_window(mini)))
	{
...
MakeNewSpot()
{
  ...
  while (TRUE)
    {
      for (mini = NORM_WINDOW;mini < MAX_BUFFERS;)
	/* "mini++" is handled by case 'n'
	   below or "else" below */
	if (!data_loaded[mini])
	  mini++;
	else
	  switch (query_split_win(mini))
	    {
	    case 'y':
	      return(unload_window(mini));
	    case 'h':
	      if (!splitwindow(mini,True)) break;
	      return(TRUE);
	    case 'v':
	      if (!splitwindow(mini,False)) break;
	      return(TRUE);
	    case 'c':
	      return(FALSE);
	    case 'n':
