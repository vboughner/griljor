h34557
s 00000/00000/00666
d D 1.17 92/08/07 20:59:37 vbo 17 16
c fixes for sun port merged in
e
s 00005/00000/00661
d D 1.16 92/03/11 20:53:17 vanb 16 15
c added to structures for containers, fixed more bugs
e
s 00052/00007/00609
d D 1.15 92/03/02 22:07:42 vanb 15 14
c finished getting editmap to work on basics
e
s 00033/00031/00583
d D 1.14 92/02/20 22:04:45 vanb 14 13
c 
e
s 00084/00080/00530
...
I 14
#define IDX(var, plus) \
  (((char *) (tobj.var)) - ((char *) &(tobj)) + (plus))/* for indexed arrays */
E 14
...
I 15
/* DON'T use negative numbers, zero, or numbers higher than 32700 as id
   numbers for object properties.  Use NULLID for the id number of non-
   variables property slots. */
E 15
I 5
...
E 14
I 14
  ADD("Map name", LSTRBOX, OFF(name), SIZ(name), NULL, NULL, 100);
E 14
  ADD("", BLANKBOX, 0, 0, NULL, NULL, NULLID);
...
E 14
I 14
  ADD("Neutrals are allowed", BOLBOX, OFF(neutrals_allowed), SIZ(neutrals_allowed), NULL, NULL, 155);
E 14
  ADD("", BLANKBOX, 0, 0, NULL, NULL, NULLID);
...
E 14
I 14
  ADD("Neutral name", LSTRBOX, IDX(team_name, 0), sizeof(char *), NULL, NULL, 145);
  ADD("Team name 1", LSTRBOX, IDX(team_name, sizeof(char*)*1), sizeof(char *), NULL, NULL, 110);
  ADD("Team name 2", LSTRBOX, IDX(team_name, sizeof(char*)*2), sizeof(char *), NULL, NULL, 120);
  ADD("Team name 3", LSTRBOX, IDX(team_name, sizeof(char*)*3), sizeof(char *), NULL, NULL, 130);
  ADD("Team name 4", LSTRBOX, IDX(team_name, sizeof(char*)*4), sizeof(char *), NULL, NULL, 140);
E 14
  ADD("", BLANKBOX, 0, 0, NULL, NULL, NULLID);

I 14
  ADD("Object file", LSTRBOX, OFF(objfilename), SIZ(objfilename), NULL, NULL, 160);
  ADD("Execution file", LSTRBOX, OFF(execute_file), SIZ(execute_file), NULL, NULL, 170);
  ADD("Startup file", LSTRBOX, OFF(startup_file), SIZ(startup_file), NULL, NULL, 180);
  ADD("Placement file", LSTRBOX, OFF(placement_file), SIZ(placement_file), NULL, NULL, 190);
  ADD("", BLANKBOX, 0, 0, NULL, NULL, NULLID);
...
E 14
I 14
  ADD("Name", LSTRBOX, OFF(name), SIZ(name), NULL, NULL, 100);
  ADD("Room is home to team", INTBOX, OFF(team), SIZ(team), NULL, NULL, 130);
  ADD("Background floor object", LINKPIC, OFF(floor), SIZ(floor), NULL, NULL, 120);
  ADD("", BLANKBOX, 0, 0, NULL, NULL, NULLID);
  ADD("Objects can appear", BOLBOX, OFF(objects_appear), SIZ(objects_appear), NULL, NULL, 160);
  ADD("Object appear floor", LINKPIC, OFF(object_floor), SIZ(object_floor), NULL, NULL, 180);
  ADD("", BLANKBOX, 0, 0, NULL, NULL, NULLID);
  ADD("People can appear", BOLBOX, OFF(people_appear), SIZ(people_appear), NULL, NULL, 170);
  ADD("People appear floor", LINKPIC, OFF(people_floor), SIZ(people_floor), NULL, NULL, 190);
  ADD("", BLANKBOX, 0, 0, NULL, NULL, NULLID);
  ADD("Starts dark", BOLBOX, OFF(dark), SIZ(dark), NULL, NULL, 110);
  ADD("Light will cycle", BOLBOX, OFF(cycles), SIZ(cycles), NULL, NULL, 140);
  ADD("Sight is limited", BOLBOX, OFF(limited_sight), SIZ(limited_sight), NULL, NULL, 150);
  ADD("", BLANKBOX, 0, 0, NULL, NULL, NULLID);
  ADD("North exits to", INTBOX, OFF(exit[0]), SIZ(exit[0]), NULL, NULL, 200);
D 15
  ADD("East exits to", INTBOX, OFF(exit[1]), SIZ(exit[1]), NULL, NULL, 200);
  ADD("South exits to", INTBOX, OFF(exit[2]), SIZ(exit[2]), NULL, NULL, 200);
  ADD("West exits to", INTBOX, OFF(exit[3]), SIZ(exit[3]), NULL, NULL, 200);
E 15
I 15
  ADD("East exits to", INTBOX, OFF(exit[1]), SIZ(exit[1]), NULL, NULL, 210);
  ADD("South exits to", INTBOX, OFF(exit[2]), SIZ(exit[2]), NULL, NULL, 220);
  ADD("West exits to", INTBOX, OFF(exit[3]), SIZ(exit[3]), NULL, NULL, 230);
E 15
E 14
E 7
  ADD("", BLANKBOX, 0, 0, NULL, NULL, NULLID);
...
DialogList *AllocMapObjectProperties()
/* returns a pointer to a newly allocated array of the properties an object
   in a map can have (that are saved in a file, anyway) */
{
  int space = 0, size = 0;
  DialogList *new = NULL;
  MapObject tobj;

  ADD("type", INTBOX, OFF(type), SIZ(type), NULL, NULL, 100);
  ADD("id (don't change)", LONGINT, OFF(id), SIZ(id), NULL, NULL, 110);

I 16
  ADD("id - 1st object contained", LONGINT, OFF(contained_id), SIZ(contained_id), NULL, NULL, 111);
  ADD("id - object containing this one", LONGINT, OFF(container_id), SIZ(container_id), NULL, NULL, 112);
  ADD("id - prior object contained with", LONGINT, OFF(lsibling_id), SIZ(lsibling_id), NULL, NULL, 113);
  ADD("id - next object contained with", LONGINT, OFF(rsibling_id), SIZ(rsibling_id), NULL, NULL, 114);

E 16
  ADD("detail", INTBOX, OFF(rec.detail), SIZ(rec.detail), NULL, NULL, 120);
  ADD("     x", INTBOX, OFF(rec.infox), SIZ(rec.infox), NULL, NULL, 130);
  ADD("     y", INTBOX, OFF(rec.infoy), SIZ(rec.infoy), NULL, NULL, 140);
  ADD("zinger", INTBOX, OFF(rec.zinger), SIZ(rec.zinger), NULL, NULL, 150);
  ADD("extra1", INTBOX, OFF(rec.extra[0]), SIZ(rec.extra[0]), NULL, NULL, 160);
  ADD("extra2", INTBOX, OFF(rec.extra[1]), SIZ(rec.extra[1]), NULL, NULL, 170);
  ADD("extra3", INTBOX, OFF(rec.extra[2]), SIZ(rec.extra[2]), NULL, NULL, 180);

  ADD("room", INTBOX, OFF(loc.room), SIZ(loc.room), NULL, NULL, 200);
  ADD("locx", INTBOX, OFF(loc.x), SIZ(loc.x), NULL, NULL, 210);
  ADD("locy", INTBOX, OFF(loc.y), SIZ(loc.y), NULL, NULL, 220);

  ADD("info follows", BOLBOX, OFF(info), SIZ(info), NULL, NULL, 300);
  ADD(NULL, ENDLIST, 0, 0, NULL, NULL, NULLID);
...
DialogList *StaticMapObjectProperties()
/* returns a pointer to the array that describes the properties that
   any particular object can be composed of.  This list must not be freed. */
{
  static DialogList *dialist = NULL;

  if (!dialist) dialist = AllocMapObjectProperties();
E 15
E 7
  return dialist;
}
