h34076
s 00000/00000/00292
d D 1.4 92/08/07 01:04:00 vbo 4 3
c source copied to a separate tree for work on new map and object format
e
s 00002/00002/00290
d D 1.3 91/08/03 23:19:41 labc-3id 3 2
c fixed a little inverseness problem with title screen ll window
e
s 00083/00082/00209
d D 1.2 91/06/16 14:16:07 labc-3id 2 1
c trevor was making some color graphics changes
e
s 00291/00000/00000
d D 1.1 91/05/17 04:16:09 labc-3id 1 0
c date and time created 91/05/17 04:16:09 by labc-3id
e
u
U
f e 0
...
D 2
  readSceneBitmaps(display, window, fg, bg, depth, river, mask,
E 2
I 2
D 3
  /* use 1 for the depth */
  readSceneBitmaps(display, window, fg, bg, 1, river, mask,
E 3
I 3
...
E 2
  ...
  for(y=0; y<=19*32; y+=32) {
    for(x=bpoints[inc][0]*32; x<=bpoints[inc][1]*32; x+=32) {
      num = randNum(3);
D 2
      XCopyArea(display, beach[num], background, context, 
		0, 0, 32, 32, x, y);
E 2
I 2
...
E 2
  ...
  for(x=0; x<=16*32; x+=32) {
    for(y=0; y<=4*32; y+=32) {
D 2
      XCopyArea(display, forest, background, context, 
		0, 0, 32, 32, x, y);
E 2
I 2
...
D 2
      XCopyArea(display, sand, background, context, 
		0, 0, 32, 32, x, y);
E 2
I 2
...
E 2
  ...
  for(inc=0; inc<6; inc++) {
    for(x=hpoints[inc][1]*32; x<=hpoints[inc][2]*32; x+=32) {
      y=hpoints[inc][0]*32;
D 2
      XCopyArea(display, path[0], background, context, 
		0, 0, 32, 32, x, y);
E 2
I 2
...
E 2
  ...
  for(inc=0; inc<5; inc++) {
    for(y=vpoints[inc][1]*32; y<=vpoints[inc][2]*32; y+=32) {
      x=vpoints[inc][0]*32;
D 2
      XCopyArea(display, path[1], background, context, 
		0, 0, 32, 32, x, y);
E 2
I 2
...
E 2
  ...
  for(inc=0; inc<4; inc++) {
D 2
    XCopyArea(display, path[2], background, context, 
	      0, 0, 32, 32, lrpoints[inc][0]*32, lrpoints[inc][1]*32);
E 2
I 2
...
E 2
  ...
  for(inc=0; inc<2; inc++) {
D 2
    XCopyArea(display, path[3], background, context, 
	      0, 0, 32, 32, llpoints[inc][0]*32, llpoints[inc][1]*32);
E 2
I 2
...
E 2
  ...
  for(inc=0; inc<2; inc++) {
D 2
    XCopyArea(display, path[4], background, context, 
	      0, 0, 32, 32, urpoints[inc][0]*32, urpoints[inc][1]*32);
E 2
I 2
...
E 2
  ...
  for(inc=0; inc<4; inc++) {
D 2
    XCopyArea(display, path[5], background, context, 
	      0, 0, 32, 32, ulpoints[inc][0]*32, ulpoints[inc][1]*32);
E 2
I 2
...
E 2
  ...
  for(inc=1; inc<r0points[0]; inc+=2) {
    num = randNum(2);
D 2
    XCopyArea(display, mask[num], background, maskGC, 
	      0, 0, 32, 32, r0points[inc]*32, r0points[inc+1]*32);
    XCopyArea(display, river[num], background, drawGC, 
	      0, 0, 32, 32, r0points[inc]*32, r0points[inc+1]*32);
E 2
I 2
...
E 2
  ...
  for(inc=1; inc<r2points[0]; inc+=2) {
    num = randNum(2);
D 2
    XCopyArea(display, mask[num+2], background, maskGC, 
	      0, 0, 32, 32, r2points[inc]*32, r2points[inc+1]*32);
    XCopyArea(display, river[num+2], background, drawGC, 
	      0, 0, 32, 32, r2points[inc]*32, r2points[inc+1]*32);
E 2
I 2
...
E 2
  ...
  for(inc=1; inc<r4points[0]; inc+=2) {
    num = randNum(2);
D 2
    XCopyArea(display, mask[num+4], background, maskGC, 
	      0, 0, 32, 32, r4points[inc]*32, r4points[inc+1]*32);
    XCopyArea(display, river[num+4], background, drawGC, 
	      0, 0, 32, 32, r4points[inc]*32, r4points[inc+1]*32);
E 2
I 2
...
E 2
  ...
  for(inc=1; inc<r6points[0]; inc+=2) {
    num = randNum(2);
D 2
    XCopyArea(display, mask[num+6], background, maskGC, 
	      0, 0, 32, 32, r6points[inc]*32, r6points[inc+1]*32);
    XCopyArea(display, river[num+6], background, drawGC, 
	      0, 0, 32, 32, r6points[inc]*32, r6points[inc+1]*32);
E 2
I 2
...
D 2
    XCopyArea(display, mask[8], background, maskGC, 
	      0, 0, 32, 32, r8points[inc]*32, r8points[inc+1]*32);
    XCopyArea(display, river[8], background, drawGC, 
	      0, 0, 32, 32, r8points[inc]*32, r8points[inc+1]*32);
E 2
I 2
...
D 2
    XCopyArea(display, mask[9], background, maskGC, 
	      0, 0, 32, 32, r9points[inc]*32, r9points[inc+1]*32);
    XCopyArea(display, river[9], background, drawGC, 
	      0, 0, 32, 32, r9points[inc]*32, r9points[inc+1]*32);
E 2
I 2
...
E 2
  ...
  for(inc=1; inc<r10points[0]; inc+=2) {
    num = randNum(2);
D 2
    XCopyArea(display, mask[10*num+10], background, maskGC, 
	      0, 0, 32, 32, r10points[inc]*32, r10points[inc+1]*32);
    XCopyArea(display, river[10*num+10], background, drawGC, 
	      0, 0, 32, 32, r10points[inc]*32, r10points[inc+1]*32);
E 2
I 2
...
D 2
    XCopyArea(display, mask[11], background, maskGC, 
	      0, 0, 32, 32, r11points[inc]*32, r11points[inc+1]*32);
    XCopyArea(display, river[11], background, drawGC, 
	      0, 0, 32, 32, r11points[inc]*32, r11points[inc+1]*32);
E 2
I 2
...
D 2
    XCopyArea(display, mask[12], background, maskGC, 
	      0, 0, 32, 32, r12points[inc]*32, r12points[inc+1]*32);
    XCopyArea(display, river[12], background, drawGC, 
	      0, 0, 32, 32, r12points[inc]*32, r12points[inc+1]*32);
E 2
I 2
...
D 2
    XCopyArea(display, mask[13], background, maskGC, 
	      0, 0, 32, 32, r13points[inc]*32, r13points[inc+1]*32);
    XCopyArea(display, river[13], background, drawGC, 
	      0, 0, 32, 32, r13points[inc]*32, r13points[inc+1]*32);
E 2
I 2
...
D 2
    XCopyArea(display, mask[14], background, maskGC, 
	      0, 0, 32, 32, r14points[inc]*32, r14points[inc+1]*32);
    XCopyArea(display, river[14], background, drawGC, 
	      0, 0, 32, 32, r14points[inc]*32, r14points[inc+1]*32);
E 2
I 2
...
D 2
    XCopyArea(display, mask[15], background, maskGC, 
	      0, 0, 32, 32, r15points[inc]*32, r15points[inc+1]*32);
    XCopyArea(display, river[15], background, drawGC, 
	      0, 0, 32, 32, r15points[inc]*32, r15points[inc+1]*32);
E 2
I 2
...
E 2
  ...
  for(inc=1; inc<r16points[0]; inc+=2) {
    num = randNum(2);
D 2
    XCopyArea(display, river[16+num], background, context, 
	      0, 0, 32, 32, r16points[inc]*32, r16points[inc+1]*32);
E 2
I 2
...
E 2
  ...
  for(x=17*32; x<=25*32; x+=32) {
    for(y=5*32; y<=6*32; y+=32) {
      num = randNum(2);
D 2
      XCopyArea(display, river[17+num], background, context, 
		0, 0, 32, 32, x, y);
E 2
I 2
