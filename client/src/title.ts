...
const R0 = [
  39, 6, 2, 5, 2, 9, 9, 11, 11, 12, 11, 13, 11, 14, 11, 15, 11, 16, 11, 18, 7, 19, 7, 20, 7, 21, 7,
  22, 7, 23, 7, 24, 7, 26, 14, 27, 14, 28, 14,
];
const R2 = [
  37, 7, 1, 9, 4, 11, 8, 13, 9, 16, 4, 17, 4, 18, 4, 19, 4, 20, 4, 21, 4, 22, 4, 23, 4, 24, 4, 25,
  4, 26, 4, 28, 11, 29, 11, 30, 11,
];
const R4 = [
  33, 6, 0, 8, 2, 10, 5, 10, 6, 10, 7, 17, 8, 17, 9, 17, 10, 28, 6, 28, 7, 28, 8, 27, 10, 32, 13,
  33, 15, 34, 18, 34, 19,
];
const R6 = [
  33, 4, 0, 4, 1, 7, 4, 7, 5, 8, 7, 8, 8, 10, 10, 14, 7, 14, 8, 15, 5, 24, 9, 24, 10, 24, 11, 25,
  13, 30, 17, 31, 19,
];
...
const R16 = [
  123, 5, 0, 5, 1, 8, 5, 9, 5, 9, 6, 9, 7, 9, 8, 11, 9, 11, 10, 12, 10, 13, 10, 14, 10, 15, 10, 16,
  10, 15, 7, 15, 8, 15, 9, 16, 5, 16, 6, 16, 7, 16, 8, 16, 9, 26, 5, 26, 6, 26, 7, 26, 8, 26, 9, 26,
  10, 26, 11, 26, 12, 26, 13, 26, 6, 26, 7, 26, 8, 27, 6, 27, 7, 27, 8, 25, 9, 25, 10, 25, 11, 26,
  12, 27, 12, 28, 12, 29, 12, 30, 12, 26, 13, 27, 13, 28, 13, 29, 13, 30, 13, 31, 13, 31, 15, 30,
  14, 31, 14, 32, 15, 31, 17, 32, 17, 32, 18, 32, 19, 33, 18, 33, 19,
];
...
function buildRiverSet(): Set<string> {
  ...
  addPts(R15);
  addPts(R16);
  for (let col = 17; col <= 25; col++) for (let row = 5; row <= 6; row++) s.add(`${col},${row}`);
  return s;
}
...
async function loadRiverBitmapMasked(url: string): Promise<ImageBitmap | null> {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      for (let i = 0; i < id.data.length; i += 4) if (id.data[i] < 200) isDark[i >> 2] = 1;
...
function riverBgAt(col: number, row: number): string {
  ...
  if (row >= 5 && row <= 9) {
    ...
    if (col >= cs && col <= ce) return 'forest';
  }
  if (row >= 8 && row <= 19) {
...
async function buildTerrainMap(tiles: Map<string, ImageBitmap | null>): Promise<OffscreenCanvas> {
  ...
  for (const [col, row] of LR_CORNERS) {
    const bm = t('lrpath');
    if (bm) ctx.drawImage(bm, col * TILE, row * TILE, TILE, TILE);
  }
  for (const [col, row] of LL_CORNERS) {
    const bm = t('llpath');
    if (bm) ctx.drawImage(bm, col * TILE, row * TILE, TILE, TILE);
  }
  for (const [col, row] of UR_CORNERS) {
    const bm = t('urpath');
    if (bm) ctx.drawImage(bm, col * TILE, row * TILE, TILE, TILE);
  }
  for (const [col, row] of UL_CORNERS) {
    const bm = t('ulpath');
    if (bm) ctx.drawImage(bm, col * TILE, row * TILE, TILE, TILE);
  }
...
export async function runTitleScreen(
  ...
  const [rawLetters, terrainBitmaps, faceSprites] = await Promise.all([
    ...
    Promise.all(
      TERRAIN_NAMES.map((n) =>
        (n.startsWith('river') ? loadRiverBitmapMasked : loadRawBitmap)(
          `/sprites/bitmaps/${n}.png`,
        ),
      ),
    ),
    ...
    for (const w of walkers) {
      ...
      if (isRiverPx(nextPx, nextPy, mapW, mapH) || isOccupied(nextPx, nextPy, w)) {
        ...
        for (const d of shuffled) {
          if (
            !isRiverPx(w.px + d.dx, w.py + d.dy, mapW, mapH) &&
            !isOccupied(w.px + d.dx, w.py + d.dy, w)
          ) {
            w.dx = d.dx;
            w.dy = d.dy;
            break;
