...
Requires `OPENWINHOME` set (for X11 headers) and paths in `legacy/src/config.h` updated to the local system. See the top-level `CLAUDE.md` for full build details.

## Original install instructions (1989)

The following are the original install instructions from the 1989 `README`, preserved verbatim:
